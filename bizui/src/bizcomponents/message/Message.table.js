...
import styles from './Message.table.less'
...
class MessageTable extends PureComponent {
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = MessageBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/message/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
