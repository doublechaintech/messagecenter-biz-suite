...
import styles from './Message.preference.less'
...
const internalImageListOf = (message) =>{
...
const internalSettingListOf = (message) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MessageService[candidateServiceName] 
...
const executeTrans = (message,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_message/doJob",payload: payload})
...
const buildTransferModal = (message,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(message,targetComponent)}
...
const internalRenderExtraHeader = (message) =>{
	return null
}
const internalRenderExtraFooter = (message) =>{
	return null
}
...
<Description term="Sender">{message.sender==null?appLocaleName(userContext,"NotAssigned"):message.sender.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Sender","profile","requestCandidateSender",
	      "transferToAnotherSender","anotherSenderId",message.sender?message.sender.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="Receiver">{message.receiver==null?appLocaleName(userContext,"NotAssigned"):message.receiver.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Receiver","profile","requestCandidateReceiver",
	      "transferToAnotherReceiver","anotherReceiverId",message.receiver?message.receiver.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
const renderPermissionSetting = message => {
  const {MessageBase} = GlobalComponents
  return <PermissionSetting targetObject={message}  targetObjectMeta={MessageBase}/>


}


class MessagePreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"Message",cardsFor: "message",cardsSource: this.props.message,
  		subItems: [
