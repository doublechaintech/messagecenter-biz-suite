...
export default {
  ...
  effects: {
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._message)
      ...
      const data = yield call(MessageService.view, payload.id)
    ...
    *load({ payload }, { call, put }) { 
      const {MessageService} = GlobalComponents;
      //yield put({ type: 'showLoading', payload })
      const data = yield call(MessageService.load, payload.id, payload.parameters)
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/message/${id}/list/${role}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, role, selectedRows, currentUpdateIndex } = payload
      const state = { id, role, selectedRows, currentUpdateIndex }
      const location = { pathname: `/message/${id}/list/${role}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/message/${id}/list/${type}List/${listName}`))
    },
