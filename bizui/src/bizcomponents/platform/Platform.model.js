...
export default {
  ...
  effects: {
    ...
    *addPrivateMessage({ payload }, { call, put }) {
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.addPrivateMessage, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *updatePrivateMessage({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.updatePrivateMessage, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *removePrivateMessageList({ payload }, { call, put }) {
     const userContext = null
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(PlatformService.removePrivateMessageList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
