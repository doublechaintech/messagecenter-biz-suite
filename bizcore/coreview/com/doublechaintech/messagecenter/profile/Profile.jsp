<%@ page import='java.util.*,com.doublechaintech.messagecenter.*,com.doublechaintech.messagecenter.profile.Profile'%>
...
<div class="row">
	...
	<% Profile result = (Profile)request.getAttribute("result");  %>
			<li><a data-toggle="tab" href="#privateMessageListAsSender" class="disabled"> ${userContext.localeMap['private_message']}</a></li>
			<li><a data-toggle="tab" href="#privateMessageListAsReceiver" class="disabled"> ${userContext.localeMap['private_message']}</a></li>
...
<sky:include page="com/doublechaintech/messagecenter/profile/Profile$Summary.jsp" />
...
<sky:include page="com/doublechaintech/messagecenter/platform/Platform$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["privateMessageListAsSender"] or ignoreListAccessControl}'>
		<c:set var="privateMessageList" value="${result.privateMessageListAsSender}" scope="request"/>
		<c:set var="privateMessageListName" value="privateMessageListAsSender" scope="request"/>
		<div id="privateMessageListAsSender" class="tab-pane fade sublist" refer-name="sender">
			<sky:include page="com/doublechaintech/messagecenter/privatemessage/PrivateMessage$List.jsp"
					referName="sender"/>
		</div>
	</c:if>
	<c:if test='${not empty userContext.accessTokens["privateMessageListAsReceiver"] or ignoreListAccessControl}'>
		<c:set var="privateMessageList" value="${result.privateMessageListAsReceiver}" scope="request"/>
		<c:set var="privateMessageListName" value="privateMessageListAsReceiver" scope="request"/>
		<div id="privateMessageListAsReceiver" class="tab-pane fade sublist" refer-name="receiver">
			<sky:include page="com/doublechaintech/messagecenter/privatemessage/PrivateMessage$List.jsp"
					referName="receiver"/>
		</div>
	</c:if>
