...
public class Platform extends BaseEntity implements  java.io.Serializable{
	...
	public static final String PRIVATE_MESSAGE_LIST                     = "privateMessageList";
	...
	protected		SmartList<PrivateMessage>	mPrivateMessageList ;
	...
	public 	Platform(String name, String introduction, String currentVersion)
	{
		...
		this.mPrivateMessageList = new SmartList<PrivateMessage>();	
	}
	...
	public  SmartList<PrivateMessage> getPrivateMessageList(){
		if(this.mPrivateMessageList == null){
			this.mPrivateMessageList = new SmartList<PrivateMessage>();
			this.mPrivateMessageList.setListInternalName (PRIVATE_MESSAGE_LIST );
			//有名字，便于做权限控制
		}
		
		return this.mPrivateMessageList;	
	}
	public  void setPrivateMessageList(SmartList<PrivateMessage> privateMessageList){
		for( PrivateMessage privateMessage:privateMessageList){
			privateMessage.setPlatform(this);
		}

		this.mPrivateMessageList = privateMessageList;
		this.mPrivateMessageList.setListInternalName (PRIVATE_MESSAGE_LIST );
	...
	public  void addPrivateMessage(PrivateMessage privateMessage){
		privateMessage.setPlatform(this);
		getPrivateMessageList().add(privateMessage);
	}
	public  void addPrivateMessageList(SmartList<PrivateMessage> privateMessageList){
		for( PrivateMessage privateMessage:privateMessageList){
			privateMessage.setPlatform(this);
		}
		getPrivateMessageList().addAll(privateMessageList);
	}
	public  void mergePrivateMessageList(SmartList<PrivateMessage> privateMessageList){
		...
		addPrivateMessageList( privateMessageList );
		
	}
	public  PrivateMessage removePrivateMessage(PrivateMessage privateMessageIndex){
		
		int index = getPrivateMessageList().indexOf(privateMessageIndex);
        if(index < 0){
        	String message = "PrivateMessage("+privateMessageIndex.getId()+") with version='"+privateMessageIndex.getVersion()+"' NOT found!";
            throw new IllegalStateException(message);
        }
        PrivateMessage privateMessage = getPrivateMessageList().get(index);        
        // privateMessage.clearPlatform(); //disconnect with Platform
        privateMessage.clearFromAll(); //disconnect with Platform
		
		boolean result = getPrivateMessageList().planToRemove(privateMessage);
        if(!result){
        	String message = "PrivateMessage("+privateMessageIndex.getId()+") with version='"+privateMessageIndex.getVersion()+"' NOT found!";
            throw new IllegalStateException(message);
        }
	...
	//断舍离
	public  void breakWithPrivateMessage(PrivateMessage privateMessage){
		
		if(privateMessage == null){
			return;
		}
		privateMessage.setPlatform(null);
		//getPrivateMessageList().remove();
	
	}
	
	public  boolean hasPrivateMessage(PrivateMessage privateMessage){
	
		return getPrivateMessageList().contains(privateMessage);
  
	}
	
	public void copyPrivateMessageFrom(PrivateMessage privateMessage) {

		PrivateMessage privateMessageInList = findThePrivateMessage(privateMessage);
		...
		getPrivateMessageList().add(newPrivateMessage);
		addItemToFlexiableObject(COPIED_CHILD, newPrivateMessage);
	}
	
	public  PrivateMessage findThePrivateMessage(PrivateMessage privateMessage){
		
		int index =  getPrivateMessageList().indexOf(privateMessage);
		//The input parameter must have the same id and version number.
		if(index < 0){
		...
		return  getPrivateMessageList().get(index);
		//Performance issue when using LinkedList, but it is almost an ArrayList for sure!
	}
	
	public  void cleanUpPrivateMessageList(){
		getPrivateMessageList().clear();
	}
	...
	public List<BaseEntity>  collectRefercencesFromLists(String internalType){
		...
		collectFromList(this, entityList, getPrivateMessageList(), internalType);
	...
	public  List<SmartList<?>> getAllRelatedLists() {
		...
		listOfList.add( getPrivateMessageList());
	...
	public List<KeyValuePair> keyValuePairOf(){
		...
		appendKeyValuePair(result, PRIVATE_MESSAGE_LIST, getPrivateMessageList());
		if(!getPrivateMessageList().isEmpty()){
			appendKeyValuePair(result, "privateMessageCount", getPrivateMessageList().getTotalCount());
			appendKeyValuePair(result, "privateMessageCurrentPageNumber", getPrivateMessageList().getCurrentPageNumber());
		}
	...
	public BaseEntity copyTo(BaseEntity baseDest){
		...
		if(baseDest instanceof Platform){
			...
			dest.setPrivateMessageList(getPrivateMessageList());
	...
	public BaseEntity mergeDataTo(BaseEntity baseDest){
		...
		if(baseDest instanceof Platform){
			...
			dest.mergePrivateMessageList(getPrivateMessageList());
